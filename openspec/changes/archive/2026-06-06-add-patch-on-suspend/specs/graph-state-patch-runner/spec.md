...
#### Scenario: suspended nodes require patch-on-suspend

- **WHEN** an interview node returns `ErrSuspended` as part of its successful business flow
- **THEN** it MUST only be migrated to runner-level `PatchNode` if the runner applies its patch through explicit patch-on-suspend semantics
